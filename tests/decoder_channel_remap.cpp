...
int main() {
    ...
    const auto stereo_bytes = opus_encode(stereo_encoder.get(), stereo_pcm.data(), frame_samples,
                                          stereo_packet.data(), static_cast<int>(stereo_packet.size()));
    ok &= expect(stereo_bytes > 0, "encode stereo packet");
    const auto mono_samples = opus_decode(mono_decoder.get(), stereo_packet.data(), stereo_bytes,
                                          mono_out.data(), frame_samples, 0);
    ...
    const auto mono_bytes = opus_encode(mono_encoder.get(), mono_pcm.data(), 320,
                                        mono_packet.data(), static_cast<int>(mono_packet.size()));
    ok &= expect(mono_bytes > 0, "encode mono packet");
    const auto stereo_samples = opus_decode(stereo_decoder.get(), mono_packet.data(), mono_bytes,
                                            stereo_out.data(), 320, 0);
    ...
    std::printf("decoder_channel_remap=PASS stereo_bytes=%d mono_bytes=%d mono_nonzero=%d stereo_nonzero=%d\n",
                stereo_bytes, mono_bytes, count_nonzero(mono_out), count_nonzero(stereo_out));
    return 0;
}

  ...
  for (int lm = 0; lm < 4; ++lm)
    for (int channels : {1, 2})
      for (int shape = 0; shape < 5; ++shape)
        for (bool allow_weak : {false, true})
          for (bool tonal : {false, true}) {
            ...
            for (int c = 0; c < channels; ++c)
              for (int i = 0; i < len; ++i) {
                ...
                float gain = shape == 0 ? 0.f : shape == 1 ? 1.f
                                            : shape == 2   ? (i > len / 2 ? 1.f : .0001f)
                                            : shape == 3   ? (i == len / 2 + c * 3 ? 1.f : 0.f)
                                                           : (i > len / 3 && i < len / 3 + 12 ? 1.f : .001f);
            ...
            if (got != ref || got_est != ref_est || got_ch != ref_ch || got_weak != static_cast<bool>(ref_weak)) {
              if (failures < 5)
                std::printf("mismatch lm=%d ch=%d shape=%d allow_weak=%d tonal=%d transient=%d/%d estimate=%.9g/%.9g channel=%d/%d weak=%d/%d\n",

...
struct Model {
  ...
  int step(bool active, int source, std::int32_t n, int endm1) {
    ...
    if (boot) {
      ...
      for (auto& [ts, b] : ev)
        if (t - ts <= W && b > expanded) expanded = b;
      if (source >= 0) ev.push_back({t, source});
    }
    t += n;
    std::vector<std::pair<long long, int>> keep;
    for (auto& e : ev)
      if (t - e.first <= W + 1) keep.push_back(e);
    ev.swap(keep);
    return expanded;
  }
};
...
void constant_20ms_boundary() {
  SignalBwTemporal st; Model m;
  const int endm1 = 20;
  for (int f = 0; f < 3; ++f) assert(step(st, m, true, -1, 960, endm1) == 20);
  assert(st.active_samples == B);
  const int seq[8] = {20, 20, 20, 20, 20, 20, 14, 14};
  for (int f = 0; f < 8; ++f) assert(step(st, m, true, 14, 960, endm1) == seq[f]);
...
void silence_and_aux_exclusion() {
  SignalBwTemporal st; Model m;
  const int endm1 = 20;
  for (int f = 0; f < 5; ++f) step(st, m, true, -1, 960, endm1);
  for (int gap = 0; gap < 5; ++gap) {
    const auto a0 = st.active_samples, l0 = st.last_frame_samples;
    int r0[7];
    for (int i = 0; i < 7; ++i) r0[i] = st.remaining[i];
    for (int k = 0; k < 50; ++k) {
    ...
    for (int i = 0; i < 7; ++i) assert(st.remaining[i] == r0[i]);
...
void downward_expiry_and_widening() {
  SignalBwTemporal st; Model m;
  const int endm1 = 20;
  for (int f = 0; f < 3; ++f) step(st, m, true, -1, 960, endm1);
  assert(step(st, m, true, 20, 960, endm1) == 20);
  assert(step(st, m, true, 14, 960, endm1) == 20);
  for (int f = 0; f < 8; ++f) step(st, m, true, 14, 960, endm1);
...
void rate_change_retention() {
  SignalBwTemporal st;
  for (int f = 0; f < 3; ++f) st.advance(true, -1, 960, 20);
  for (int f = 0; f < 7; ++f) st.advance(true, -1, 960, 20);
  int floor = 18;
...
void resets() {
  SignalBwTemporal st; Model m;
  for (int f = 0; f < 30; ++f) step(st, m, true, 18, 960, 20);
  st.reset(); m.reset();
  assert(st.active_samples == 0 && st.last_frame_samples == 0);
  for (int i = 0; i < 7; ++i) assert(st.remaining[i] == 0);
  for (int f = 0; f < 10; ++f) step(st, m, true, -1, 480, 20);

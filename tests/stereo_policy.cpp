...
bool check(settings config, int pattern, bool should_activate) {
  ...
  if (!enc || !dec || status != OPUS_OK) return false;
  if (opus_encoder_ctl(enc.get(), OPUS_SET_BITRATE(64000)) || opus_encoder_ctl(enc.get(), OPUS_SET_COMPLEXITY(config.complexity)) ||
      ...
      opus_encoder_ctl(enc.get(), OPUS_SET_VBR(config.vbr))) return false;
  ...
  unsigned random = 1;
  const auto tone = [](double t) { return .18 * std::sin(6.283185307179586 * 233.08 * t) + .10 * std::sin(6.283185307179586 * 698.46 * t); };
  for (int frame = 0; frame <= 150; ++frame) {
    if (frame == 150 && opus_encoder_ctl(enc.get(), OPUS_RESET_STATE) != OPUS_OK) return false;
    for (int i = 0; i < framesize; ++i) {
      ...
      if (pattern == 1) right = left;
      if (pattern == 2) right = -left;
      if (pattern == 3) right = tone(t - 73. / config.rate);
      ...
      if (pattern == 5) left = right = 0;
      input[i * config.channels] = static_cast<opus_int16>(std::lrint(32767 * left));
      if (config.channels == 2) input[i * 2 + 1] = static_cast<opus_int16>(std::lrint(32767 * right));
    }
    const int bytes = opus_encode(enc.get(), input.data(), framesize, packet.data(), packet.size());
    if (bytes <= 0 || opus_decode(dec.get(), packet.data(), bytes, output.data(), framesize, 0) != framesize) return false;
    if (frame == 0) first_packet.assign(packet.begin(), packet.begin() + bytes);
    if (frame == 150) {
      if (encoder_celt_state(enc.get())->stereo_policy_celt || enc->stereo_recovery_frames != 0) return false;
      if (pattern != 4 && (bytes != static_cast<int>(first_packet.size()) || !std::equal(first_packet.begin(), first_packet.end(), packet.begin()))) return false;
    } else {
...
int main() {
  if (!check({}, 0, true)) return 1;
  for (int pattern = 1; pattern <= 5; ++pattern) if (!check({}, pattern, false)) return 1;
  for (const auto config : std::array{settings{.complexity=0}, settings{.complexity=4}, settings{.complexity=5}, settings{.complexity=8}, settings{.rate=24000}, settings{.frame_ms=10},
    ...
    if (!check(config, 0, false)) return 1;
  }

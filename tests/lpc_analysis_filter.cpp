...
static bool check_silk_reconstruction() {
  for (const int complexity : {0, 5, 9, 10}) {
    for (const int bitrate : {16000, 24000, 32000, 48000}) {
      ...
      if (!encoder || !decoder || error || opus_encoder_ctl(encoder.get(), OPUS_SET_BITRATE(bitrate)) || opus_encoder_ctl(encoder.get(), OPUS_SET_COMPLEXITY(complexity))) return false;
      ...
      for (int frame = 0; frame < 200; ++frame) {
        ...
        if (bytes <= 0 || opus_decode(decoder.get(), packet.data(), bytes, output.data(), 320, 0) != 320) return false;
        ...
        if (state.fs_kHz != 16 || encoder->mode != opus_mode_silk_only || decoded_state.resampler_state.inputDelay + 1 != static_cast<int>(tail.size())) return false;
        const auto* samples = state.sNSQ.xq + state.ltp_mem_length - state.frame_length;
        if (frame >= 10) {
          for (std::size_t i = 0; i < output.size(); ++i) {
            if (output[i] != (i < tail.size() ? tail[i] : samples[i - tail.size()])) return false;
          }
        }
...
int main() {
  ...
  const auto check_fraction = [](opus_uint32 bits) {
    ...
    for (auto value = bits; value != 0; value >>= 1) --expected_leading;
  ...
  if (!check_fraction(0)) return 1;
  for (unsigned bits = 0; bits < 32; ++bits) {
    if (low_bits_mask(bits) != mask || !check_fraction(opus_uint32{1} << bits) || !check_fraction(mask)) return 1;
    mask = (mask << 1) | 1;
  }
  for (int i = 0; i < 200000; ++i) {
    random = random * 1664525U + 1013904223U;
    if (!check_fraction(random)) return 1;
  }
  int status = OPUS_OK;
  ...
  if (!encoder || status != OPUS_OK || opus_encoder_ctl(encoder.get(), OPUS_SET_BITRATE(16000)) != OPUS_OK) return 1;
  std::array<opus_int16, 960> speech{};
  std::array<unsigned char, 1500> packet{};
  for (int i = 0; i < 960; ++i) speech[i] = static_cast<opus_int16>((i % 50 - 25) * 200);
  if (opus_encode(encoder.get(), speech.data(), 960, packet.data(), packet.size()) <= 0 || encoder->mode == opus_mode_celt_only) return 1;
  ...
  for (const auto expected : {opus_uint32{0}, opus_uint32{1}}) {
    if (opus_encode(encoder.get(), speech.data(), 960, packet.data(), packet.size()) <= 0 || state.frameCounter != expected) return 1;
  }
  for (int order : {0, 2, 4, 6, 8, 10, 12, 16, 24}) {
    ...
    if (silk_schur_FLP(reflection.data(), correlation.data(), order) != .75f) return 1;
    for (int i = 0; i < 24; ++i) {
      if (reflection[i] != (i < order ? 0.f : 1.f)) return 1;
    }
  }
  ...
  for (int samples : {240, 360, 600, 1080}) {
    ...
    denominator[0] = -.5f;
    memory[0] = .25f;
    for (int i = 0; i < samples; ++i) pcm[i] = static_cast<float>(i % 17 - 8) * .125f;

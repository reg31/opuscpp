...
[[nodiscard]] auto find_clip(const std::vector<clip_input>& clips, std::string_view label, int channels) -> const clip_input* {
...
[[nodiscard]] auto make_official_decoder(int channels) -> std::unique_ptr<OpusDecoder, decltype(&opus_decoder_destroy)> {
  int error = OPUS_OK;
...
[[nodiscard]] auto describe_packet_mismatch(std::span<const unsigned char> expected, std::span<const unsigned char> actual,
                                            std::uint32_t expected_range, std::uint32_t actual_range, const clip_input& clip,
                                            const scenario& current, std::size_t frame_index) -> std::string {
  const auto mismatch = std::ranges::mismatch(
      expected, actual, std::ranges::equal_to{},
      [](unsigned char value) {
        return value;
      },
      [](unsigned char value) {
  ...
  auto append_slice = [&message](std::string_view label, std::span<const unsigned char> bytes, std::size_t begin, std::size_t end) {
    message += " ";
    message += label;
    message += "=";
...
void configure_encoder(codex_OpusEncoder* encoder, int channels, const scenario& current) {
  if (const auto ret = codex_opus_encoder_ctl(encoder, OPUS_SET_BITRATE_REQUEST, scenario_bitrate(current, channels)); ret != OPUS_OK) {
    throw std::runtime_error("failed to set codex bitrate");
  }
  if (const auto ret = codex_opus_encoder_ctl(encoder, OPUS_SET_COMPLEXITY_REQUEST, current.complexity); ret != OPUS_OK) {
    throw std::runtime_error("failed to set codex complexity");
  }
  if (const auto ret = codex_opus_encoder_ctl(encoder, OPUS_SET_VBR_REQUEST, static_cast<int>(current.vbr)); ret != OPUS_OK) {
...
[[nodiscard]] auto compare_case(const case_record& validation_case, const clip_input& clip, const scenario& current) -> std::string {
  ...
  for (std::size_t frame_index = 0; frame_index < validation_case.frames.size(); ++frame_index) {
    ...
    const int packet_size =
        codex_opus_encode(encoder.get(), samples.data() + frame_offset, current.frame_size, packet.data(), static_cast<int>(packet.size()));
    ...
    if (const auto ret = codex_opus_encoder_ctl(encoder.get(), OPUS_GET_FINAL_RANGE_REQUEST, &final_range); ret != OPUS_OK) {
    ...
    if (require_bitexact) {
      ...
      if (!std::ranges::equal(std::span<const unsigned char>{packet.data(), static_cast<std::size_t>(packet_size)},
                              std::span<const unsigned char>{validation_frame.packet})) {
        throw std::runtime_error(describe_packet_mismatch(
            validation_frame.packet, std::span<const unsigned char>{packet.data(), static_cast<std::size_t>(packet_size)},
            validation_frame.final_range, final_range, clip, current, frame_index));
      }
      if (final_range != validation_frame.final_range) {
        throw std::runtime_error("final range mismatch for " + clip.label + " / " + std::string{current.name} + " frame " +
                                 std::to_string(frame_index));
      }
    } else {
      const int decoded_samples = opus_decode(official_decoder.get(), packet.data(), packet_size, decoded.data(), current.frame_size, 0);
      if (decoded_samples != current.frame_size) {
        throw std::runtime_error("official decoder rejected codex packet for " + clip.label + " / " + std::string{current.name} +
    ...
    if (can_use_internal_parallelism) {
      ...
      if (internal_worker_count > 1) {
        ...
        for (int internal_worker = 0; internal_worker < internal_worker_count; ++internal_worker) {
          futures.push_back(std::async(std::launch::async, [&, internal_worker] {
            return run_slice(make_worker_slice(validation_cases.size(), internal_worker, internal_worker_count));
          }));
        }

...
[[nodiscard]] auto load_wav(const fs::path& path, double max_seconds) -> clip_data {
  ...
  for (std::size_t i = 0; i < samples.size(); ++i) {
    samples[i] = static_cast<std::int16_t>(read_u16(bytes, data_offset + i * 2));
  }
...
[[nodiscard]] auto make_celt_metric_tables() -> celt_metric_tables {
  ...
  for (int i = 0; i < celt_metric_tables::window_size; ++i) {
    tables.window[static_cast<std::size_t>(i)] =
        0.5 - 0.5 * std::cos((2.0 * pi * static_cast<double>(i)) / (celt_metric_tables::window_size - 1));
  }
  for (int bin = 0; bin < celt_metric_tables::freq_bins; ++bin) {
    ...
    auto band = 0;
    while (band + 1 < static_cast<int>(bands.size()) && bin >= bands[static_cast<std::size_t>(band + 1)]) {
      ++band;
    }
...
void add_celt_perceptual_metrics(totals& out, std::span<const std::int16_t> ref, std::span<const float> deg, int channels) {
  ...
  for (std::size_t start = 0; start + celt_metric_tables::window_size <= frames; start += celt_metric_tables::window_size) {
    ...
    for (int band = 1; band < nbands; ++band) {
      for (int ch = 0; ch < channels; ++ch) {
        mask[static_cast<std::size_t>(band)][static_cast<std::size_t>(ch)] +=
            0.1 * mask[static_cast<std::size_t>(band - 1)][static_cast<std::size_t>(ch)];
      }
    }
    for (int band = nbands - 1; band-- > 0;) {
      for (int ch = 0; ch < channels; ++ch) {
        mask[static_cast<std::size_t>(band)][static_cast<std::size_t>(ch)] +=
            0.03 * mask[static_cast<std::size_t>(band + 1)][static_cast<std::size_t>(ch)];
      }
    }
    if (out.celt_windows != 0) {
      for (int band = 0; band < nbands; ++band) {
        for (int ch = 0; ch < channels; ++ch) {
          mask[static_cast<std::size_t>(band)][static_cast<std::size_t>(ch)] +=
              0.5 * previous_mask[static_cast<std::size_t>(band)][static_cast<std::size_t>(ch)];
        }
      }
    }
    if (channels == 2) {
...
void add_metrics(totals& out, std::span<const std::int16_t> ref, std::span<const float> deg, int channels) {
  ...
  for (std::size_t i = 0; i < centers.size(); ++i) {
    coeffs[i] = 2.0 * std::cos(2.0 * pi * centers[i] / sample_rate);
  }
  auto window = std::array<double, frame_size>{};
  for (std::size_t i = 0; i < window.size(); ++i) {
    window[i] = 0.5 - 0.5 * std::cos(2.0 * pi * static_cast<double>(i) / (window.size() - 1));
  }
...
void write_u32(std::ostream& out, std::uint32_t v) {
  const std::array<unsigned char, 4> b{static_cast<unsigned char>(v), static_cast<unsigned char>(v >> 8),
                                       static_cast<unsigned char>(v >> 16), static_cast<unsigned char>(v >> 24)};
  out.write(reinterpret_cast<const char*>(b.data()), 4);
}

template <typename Getter> void write_wav(const fs::path& path, int channels, std::size_t samples, Getter get) {
  ...
  for (std::size_t i = 0; i < samples; ++i) {
    write_u16(out, static_cast<std::uint16_t>(get(i)));
  }
}
...
void write_listening(const options& opt, const clip_data& clip, const result& current, const result& official) {
  ...
  write_wav(base.string() + "_current.wav", clip.channels, current.decoded.size(), [&](std::size_t i) {
    return static_cast<std::int16_t>(
        std::clamp<long>(std::lrint(std::clamp(static_cast<double>(current.decoded[i]), -1.0, 1.0) * 32767.0), -32768, 32767));
  });
  write_wav(base.string() + "_official.wav", clip.channels, official.decoded.size(), [&](std::size_t i) {
    return static_cast<std::int16_t>(
        std::clamp<long>(std::lrint(std::clamp(static_cast<double>(official.decoded[i]), -1.0, 1.0) * 32767.0), -32768, 32767));
  });
}
...
[[nodiscard]] auto delta(std::uint64_t after, std::uint64_t before) noexcept -> std::uint64_t {
...
auto measure_group(std::string_view label, int count, Factory factory, int api_state_bytes = 0) -> std::vector<state_handle> {
  ...
  std::cout << "memory label=" << label << " instances=" << count << " private_delta_bytes=" << private_delta
            << " private_per_instance=" << std::fixed << std::setprecision(1) << (static_cast<double>(private_delta) / count)
            << " working_set_delta_bytes=" << working_delta << " working_set_per_instance=" << (static_cast<double>(working_delta) / count);
  if (api_state_bytes > 0) {
    std::cout << " official_api_state_bytes=" << api_state_bytes;
  }
  std::cout << '\n';
  return states;
}
...
void run_memory(const options& opt) {
  ...
  for (const auto channels : {1, 2}) {
    ...
    retained.push_back(measure_group("current_decoder_ch" + std::to_string(channels), opt.memory_instances, [&] {
      return make_current_decoder(channels);
    }));
    retained.push_back(measure_group(
        "official_decoder_ch" + std::to_string(channels), opt.memory_instances,
        [&] {
          return make_official_decoder(channels);
        },
        opus_decoder_get_size(channels)));
  }
  const auto mem = read_memory();
  std::cout << "memory retained_groups=" << retained.size() << " final_private_bytes=" << mem.private_bytes
            << " final_working_set_bytes=" << mem.working_set << " peak_working_set_bytes=" << mem.peak_working_set << '\n';
}
...
[[nodiscard]] auto parse_options(int argc, char** argv) -> options {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--input") {
      ...
      opt.listening_dir = fs::path{value()};
    else if (arg == "--memory-instances")
  ...
  if (opt.memory_instances < 1) {
    throw std::runtime_error("--memory-instances must be positive");
  }
...
void run_quality(const options& opt) {
  ...
  std::cout << "  delta current_minus_official"
            ...
            << " packet_bytes=" << (avg_packet_bytes(current.score) - avg_packet_bytes(official.score)) << " packet_bytes_pct="
            << (100.0 * (avg_packet_bytes(current.score) - avg_packet_bytes(official.score)) /
                std::max(1e-9, avg_packet_bytes(official.score)))
            << " encode_speed_ratio_current_vs_official=" << (encode_ms(current.score) / std::max(1e-9, encode_ms(official.score))) << '\n';
  if (!opt.listening_dir.empty()) {
    std::cout << "listening_wavs=" << opt.listening_dir.string() << '\n';
  }
}

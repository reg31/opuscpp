  ...
  for (int lm = 0; lm <= 3; ++lm)
    for (int profile = 0; profile < 3; ++profile) {
      ...
      for (int band = 0; band < celt_default_nb_ebands; ++band) {
        const float left = profile == 0 ? -9.f : profile == 1 ? 0.f
                                                              : -static_cast<float>(band % 8);
        const float right = profile == 0 ? 0.f : profile == 1 ? -9.f
                                                              : -static_cast<float>((band + 4) % 8);
      ...
      if (bytes <= 0 || celt_decode_with_ec(dec, packet.data(), bytes, output.data(), n, nullptr, nullptr) != n)
        return 2;

...
bool ctl_ok(OpusEncoder* st, int request, auto&&... args) {
  return opus_encoder_ctl(st, request, args...) == 0;
}
...
auto measure(int frame_size, int application, int bitrate, int frames, sample_fn fn) -> outcome {
  ...
  if (!ctl_ok(enc.get(), set_bitrate, bitrate) || !ctl_ok(enc.get(), set_complexity, 10))
    return {false, 0, 0, 0, -1};
  int lookahead = 0;
  if (!ctl_ok(enc.get(), get_lookahead, &lookahead) || lookahead < 0)
    return {false, 0, 0, 0, -1};

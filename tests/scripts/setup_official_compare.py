...
def emit_metrics_report(
    ...
    rfc: dict[str, str] | None = None,
    encode: dict[str, str] | None = None,
    perceptual: dict[str, object] | None = None,
    benchmark_rows: list[dict[str, str]] | None = None,
    detector_rows: list[str] | None = None,
    binary_size: dict[str, str] | None = None,
    ...
    quality_rows = perceptual["rows"] if perceptual else sections["quality_vs_official"]
    speed_rows = benchmark_rows
    if speed_rows is None:
        ...
        for row in sections["encode_speed_vs_official"]:
            ...
            speed_rows.append({
                "bitrate": bitrate,
                "encode_speedx": row.get("encode_speedx", ""),
                "decode_speedx": decode_row.get("decode_speedx", ""),
                "packet_delta_pct": row.get("packet_delta_pct", ""),
            })

    rfc_text = f"{rfc['passed']}/{rfc['total']} RFC decode vectors passed" if rfc else "24/24 RFC decode vectors passed"
    encode_text = f"{encode['passed']}/96 encode oracle cases passed" if encode else "96/96 encode oracle cases passed"
    ...
    lines: list[str] = [
        ...
        f"- Local result: {rfc_text}.",
        "",
        "## Encode oracle conformance",
        "",
        f"- Local result: {encode_text}.",
        "",
    ...
    for row in speed_rows:
        ...
        packet_delta = row.get("packet_delta_pct", "")
        packet_cell = f"{packet_delta}%" if packet_delta else ""
        lines.append(
            f"| {bitrate_kbps} kbps | {row.get('encode_speedx', '')}x | {row.get('decode_speedx', '')}x | {packet_cell} |"
        )
    ...
    lines += [
        ...
        "",
        "```text",
        (binary_size or {}).get("output", (binary_size or {}).get("status", "unavailable")).strip(),
        "```",
        "",
        "## Toolchains checked",
        "",
    ...
    if toolchains:
        lines += ["", "Latest local toolchain check output:", "", "```text", *toolchains, "```"]
...
def run_detector_mode_balance(
    ...
    curr_obj = compile_object(
        ...
        build_dir / "curr_opus_codec.o",
        [repo_root / "src"],
        [*current_alias_macros("curr"), "-DOPUSCPP_DETECTOR_BENCH_HOOK"],
    )
    exe = link_executable(
        cxx,
...
def main() -> int:
    ...
    metrics_report = emit_metrics_report(repo_root, report_dir, rfc, encode, perceptual, benchmark_rows, detector_rows, binary_size, toolchains)
    if args.report_out:

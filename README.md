...
- Host MinGW GCC `-O2` binary text in the current measured snapshot: `261,692 B` (`279,780 B` text+data+bss).
...
| Encode is faster than official Opus with x86 intrinsics in 9/9 measured bitrates in the current run (1.46x to 2.43x). | Supports a documented subset of the full Opus CTL/API surface. |
| Decode is faster than official Opus with x86 intrinsics in 9/9 measured bitrates in the current run (1.06x to 1.72x). | A few high-rate decode points are close to parity and official Opus remains extremely mature. |
...
| 24&nbsp;kbps | +0.0109 | +0.0095 | 23.928 kbps | 24.148 kbps |

...
constexpr int encoder_max_stage_samples = (3 * celt_max_frame_samples + celt_sample_rate / 100) * celt_max_channels;
...
[[nodiscard]] static auto silk_lpc_prediction_q10_fixed(const opus_int32* history_end, const opus_int16* coefficients) noexcept -> opus_int32 {
...
struct CeltEncoderInternal {
  ...
  quality_history_state* quality_history;
  opus_uint32 rng;
...
constexpr int quality_input_delay_48k_audio = 312;
...
enum class quality_tracking_status : int {
  waiting = 1,
  valid = 2,
  unsupported_transition = 3,
  unsupported_configuration = 4,
  packet_rewritten = 5,
  hybrid_only = 6,
  unsupported_redundancy = 7,
  silk_only = 8
};
struct quality_history_state {
  ...
  unsigned frames = 0;
  int packet_prev_mode = 0;
  bool ready = false, packet_selection_ready = false, previous_packet_celt = false;
  bool packet_prev_redundancy = false, optional_epoch_invalid = false;
  quality_tracking_status status = quality_tracking_status::waiting;
};
static unsigned char quality_payload_byte(std::span<const unsigned char> payload, std::size_t index) {
  return index < payload.size() ? payload[index] : 0;
}
static bool quality_resume_decoder(const ec_enc& checkpoint, std::span<unsigned char> payload, ec_dec& decoder) {
  if (checkpoint.end_offs != 0 || checkpoint.end_window != 0 || checkpoint.nend_bits != 0 || checkpoint.nbits_total < 33 ||
      ((checkpoint.nbits_total - 33) & 7) != 0 || checkpoint.rng <= ec_code_bot)
    return false;
  const auto k = static_cast<std::size_t>((checkpoint.nbits_total - 33) / 8);
  opus_uint32 word = static_cast<opus_uint32>(quality_payload_byte(payload, k)) << 24;
  word |= static_cast<opus_uint32>(quality_payload_byte(payload, k + 1)) << 16;
  word |= static_cast<opus_uint32>(quality_payload_byte(payload, k + 2)) << 8;
  word |= quality_payload_byte(payload, k + 3);
  decoder = {};
  decoder.buf = payload.data();
  decoder.storage = static_cast<opus_uint32>(payload.size());
  decoder.offs = static_cast<opus_uint32>(std::min(k + 4, payload.size()));
  decoder.rng = checkpoint.rng;
  decoder.val = (checkpoint.val + checkpoint.rng - 1 - (word >> 1)) & ec_code_mask;
  decoder.nbits_total = checkpoint.nbits_total;
  decoder.rem = quality_payload_byte(payload, k + 3);
  return true;
}
static void quality_history_reset(quality_history_state&, int, int);
static void quality_receiver_reset(quality_history_state&);
static void quality_history_invalidate(quality_history_state&, quality_tracking_status);
...
static void quality_restore_celt(OpusDecoder*, const quality_celt_snapshot&);
static void quality_commit_history(quality_history_state&, const quality_frame_work&);
...
static int celt_encode_with_ec(CeltEncoderInternal* st, const opus_res* pcm, int frame_size, unsigned char* compressed, int nbCompressedBytes, ec_enc* enc, bool protect_transients = false, bool retain_main_shadow = true);
...
static int ref_opus_packet_parse_impl(const unsigned char* data, opus_int32 len, const unsigned char* frames[48], opus_int16 size[48], int* payload_offset) {
...
[[nodiscard]] static auto silk_CLZ32(opus_int32 value) noexcept -> opus_int32 {
  return std::countl_zero(std::bit_cast<opus_uint32>(value));
}
...
static auto silk_CLZ_FRAC(opus_int32 value, opus_int32* lz, opus_int32* frac_Q7) noexcept -> void {
  ...
  *lz = leading_zeros;
  const auto normalized = std::bit_cast<opus_uint32>(value) << (leading_zeros & 31);
  *frac_Q7 = static_cast<opus_int32>((normalized >> 24) & 0x7fU);
}

[[nodiscard]] static inline auto silk_varq_shift(opus_int32 value, int lshift) noexcept -> opus_int32 {
  return lshift <= 0 ? saturating_left_shift(value, -lshift) : lshift < 32 ? value >> lshift
                                                                           : 0;
}
...
static int celt_decode_then_add(CeltDecoderInternal* celt_dec, const unsigned char* data, int len, opus_res* pcm, int frame_size, ec_dec* dec, int channels) {
...
static int opus_packet_get_mode(const unsigned char* data) {
  return data[0] & 0x80 ? opus_mode_celt_only : (data[0] & 0x60) == 0x60 ? opus_mode_hybrid
                                                                         : opus_mode_silk_only;
}
...
static int opus_decode_frame(OpusDecoder* st, const unsigned char* data, opus_int32 len, opus_res* pcm, int frame_size, int decode_fec) {
  ...
  if (mode != opus_mode_celt_only) {
    ...
    for (; decoded_samples < frame_size;) {
      ...
      const int silk_ret = silk_Decode(silk_dec, &dec_control, lost_flag, first_frame, &dec, pcm_ptr, &silk_frame_size);
      if (silk_ret) {
        if (lost_flag) {
...
static int decode_native_celt_direct_fast(OpusDecoder* st, const unsigned char* data, opus_int32 len, opus_res* pcm, opus_int16* pcm16,
  ...
  int payload_offset;
  const int frame_count = ref_opus_packet_parse_impl(data, len, nullptr, frame_lengths.data(), &payload_offset);
  if (frame_count < 0 || frame_count * packet_frame_size > frame_size) {
    return opus_decode_fast_unavailable;
  }
...
static int decode_to_output(OpusDecoder* st, const unsigned char* data, opus_int32 len, Sample* pcm, int frame_size, int decode_fec) {
  ...
  if constexpr (std::same_as<Sample, float>) {
    ...
    return fast_ret != opus_decode_fast_unavailable ? fast_ret : decode_native(st, data, len, pcm, frame_size, decode_fec);
  } else {
...
consteval auto make_silk_nlsf_cb(const int vectors, const int order, const int quant_step_q16, const int inv_quant_step_q6, const auto& cb1_q8, const auto& weights_q9, const auto& cb1_icdf, const auto& pred_q8, const auto& ec_sel, const auto& ec_icdf, const auto& ec_rates_q5, const auto& delta_min_q15) -> silk_NLSF_CB_struct {
...
struct OpusEncoder {
  ...
  opus_uint8 stereo_similarity_ms_Q1;
  opus_uint8 stereo_recovery_frames;
...
static void release_quality_history(CeltEncoderInternal* celt) noexcept {
  if (celt != nullptr && celt->quality_history != nullptr) {
    opus_decoder_destroy(celt->quality_history->decoder);
    std::destroy_at(celt->quality_history);
    std::free(celt->quality_history);
    celt->quality_history = nullptr;
  }
}
...
static void ref_opus_encoder_init(OpusEncoder* st, opus_int32 Fs, int channels, int application) {
  ...
  std::uninitialized_value_construct_n(encoder_delay_buffer(st), encoder_delay_buffer_count(channels, application));
  ...
  celt_enc->voip_application = application == OPUS_APPLICATION_VOIP;
  st->use_vbr = 1;
  st->vbr_constraint = 1;
...
static void silk_biquad_res(const opus_res* in, opus_int32 r_Q28, const opus_int32* A_Q28, opus_val32* S, opus_res* out, const opus_int32 len, int stride) {
...
struct frame_activity_metrics {
  int is_silence;
  opus_val32 energy;
  opus_val32 mono_diff_ratio, mono_zero_cross_rate, stereo_coherence;
  bool stereo_shape_match;
};
...
static frame_activity_metrics measure_frame_activity(const opus_res* pcm, int frame_size, int channels, int lsb_depth,
  ...
  opus_val32 right_difference_energy = 0, left_difference_energy = 0;
  ...
  for (int i = 0; i < frame_size; ++i) {
    ...
    if constexpr (MeasureWidth) {
      ...
      yy += right * right;
      if (i != 0) {
        const auto right_delta = right - pcm[(i - 1) * channels + channels - 1];
        right_difference_energy += right_delta * right_delta;
        const auto left_delta = left - pcm[(i - 1) * channels];
        left_difference_energy += left_delta * left_delta;
      }
    }
  }
  if constexpr (MeasureWidth) {
  ...
  return {sample_max <= static_cast<opus_val16>(1) / (1 << lsb_depth), energy / sample_count,
          ...
          static_cast<opus_val32>(zero_crossings) / std::max(1, frame_size - 1), predictability, shape_match};
}
...
static int update_lightweight_voice_estimate(OpusEncoder* st, opus_val16 stereo_width, const frame_activity_metrics& frame_metrics) noexcept {
  ...
  return voice_score > 44 && voice_score + 16 >= music_score ? 115 : music_score >= voice_score + 8 && music_score > 36 ? 0
                                                                                                                        : 48;
}
...
[[nodiscard]] static opus_int32 quality_bandwidth_threshold(int voice_weight, opus_int32 music_threshold, opus_int32 voice_threshold) noexcept {
...
struct encoder_stage_storage {
  ...
  auto finish_frame(OpusEncoder* st) noexcept -> void {
    if (history_samples != 0) {
...
[[nodiscard]] inline auto make_encoder_stage_storage(std::span<opus_res> storage, const OpusEncoder* st, const int total_buffer, const int frame_size) noexcept -> encoder_stage_storage {
  const auto window_samples = static_cast<std::size_t>((st->encoder_buffer + frame_size) * st->channels);
  return {storage.first(window_samples),
          static_cast<std::size_t>(st->encoder_buffer * st->channels),
          static_cast<std::size_t>((st->encoder_buffer - total_buffer) * st->channels)};
}
}

[[nodiscard]] static auto encode_low_rate_packet(OpusEncoder* st, const int frame_size, const opus_int32 out_data_bytes, opus_int32 max_data_bytes, unsigned char* data) -> opus_int32 {
  int tocmode = st->mode;
...
[[nodiscard]] static auto encode_multiframe_packet(OpusEncoder* st, const opus_res* pcm, const int frame_size, unsigned char* data, const opus_int32 out_data_bytes, const multiframe_encode_params& params, std::array<opus_res, encoder_max_stage_samples>& stage_buffer_storage) -> opus_int32 {
  ...
  for (int frame_index = 0; frame_index < nb_frames; ++frame_index) {
    ...
    int tmp_len =
        opus_encode_frame_native(st, frame_pcm, enc_frame_size, curr_data, curr_max, allocator_target_bits, frame_metrics,
    ...
    if (emit_dtx) {
      ...
      if (history != nullptr)
        quality_history_invalidate_optional(*history, quality_tracking_status::packet_rewritten);
    }
    if (tmp_len == 1) {
      ++dtx_count;
    }
...
[[nodiscard]] static constexpr auto allow_stereo_policy(const OpusEncoder* st, int frame_size) noexcept -> bool {
  return st->application == OPUS_APPLICATION_AUDIO && st->channels == 2 && st->Fs == 48000 && frame_size == 960 &&
         ...
         st->use_vbr && st->vbr_constraint && st->bitrate_bps >= 48000 && st->bitrate_bps < 112000;
}

static opus_int32 encode_native(OpusEncoder* st, const opus_res* pcm, int frame_size, unsigned char* data, opus_int32 out_data_bytes, int lsb_depth, bool float_api) {
  ...
  celt_enc->stereo_policy_celt = false;
  ...
  if (first) {
    celt_enc->stereo_coherence_Q8 = 255;
  }
  if (st->application == OPUS_APPLICATION_AUDIO && st->channels == 2 && frame_metrics.stereo_coherence >= 0) {
    const int elapsed = 2000 * frame_size / st->Fs;
    st->stereo_similarity_ms_Q1 = static_cast<opus_uint8>(frame_metrics.stereo_shape_match ? std::min(240, st->stereo_similarity_ms_Q1 + elapsed) : std::max(0, st->stereo_similarity_ms_Q1 - 2 * elapsed));
    const int estimate = static_cast<int>(255 * frame_metrics.stereo_coherence + .5f);
    const int previous = celt_enc->stereo_coherence_Q8;
    celt_enc->stereo_coherence_Q8 = static_cast<opus_uint8>((3 * previous + estimate + 2) / 4);
  }
  if (st->application == OPUS_APPLICATION_AUDIO && st->channels == 2 && frame_metrics.energy > 1e-7f) {
    if (frame_metrics.mono_diff_ratio >= .025f || frame_metrics.mono_zero_cross_rate >= .075f) {
      st->stereo_recovery_frames = previous_stereo_policy || (st->stereo_recovery_frames > 0 && st->stereo_recovery_frames < 255) ? 10 : 255;
    } else if (st->stereo_recovery_frames > 0 && st->stereo_recovery_frames < 255) {
      --st->stereo_recovery_frames;
    }
  }
  ...
  if (voip_style && st->channels == 1 && (st->voip_noise_confidence_Q7 >= voip_noise_confidence_apply_Q7 || severe_voip_noise)) {
    st->mode = st->bitrate_bps <= 24000 ? opus_mode_silk_only : st->bitrate_bps <= 48000 ? opus_mode_hybrid
                                                                                         : st->mode;
  }
  ...
  if (emit_dtx && celt_enc->quality_history != nullptr) {
    quality_history_invalidate_optional(*celt_enc->quality_history, quality_tracking_status::packet_rewritten);
  }
  if (encoder_is_in_dtx(st)) {
    reset_vbr_budget(st);
...
static opus_int32 opus_encode_frame_native(OpusEncoder* st, const opus_res* pcm, int frame_size, unsigned char* data, opus_int32 orig_max_data_bytes, opus_int32 allocator_target_bits, const frame_activity_metrics& metrics, int redundancy, int celt_to_silk, int prefill, opus_int32 equiv_rate, int to_celt, bool nonfinal_frame, bool skip_celt_for_dtx, encoder_stage_storage& stage_storage) {
  ...
  auto* celt_enc = encoder_celt_state(st);
  const unsigned quality_frames_before = celt_enc->quality_history != nullptr ? celt_enc->quality_history->frames : 0;
  if (auto* history = celt_enc->quality_history; history != nullptr) {
    if (st->mode == opus_mode_silk_only) {
      quality_history_invalidate(*history, quality_tracking_status::silk_only);
    }
  }
  ...
  if (st->mode != opus_mode_celt_only) {
    ...
    if (st->mode == opus_mode_silk_only) {
      ...
      st->silk_mode.maxInternalSampleRate = effective_max_rate < 7000 ? 8000 : effective_max_rate < 8000 ? 12000
                                                                                                         : 16000;
    ...
    if (prefill) {
      ...
      gain_fade(stage_storage.active.data() + prefill_offset, stage_storage.active.data() + prefill_offset, prefill == 3 ? .75f : 0.f, 1.0f, st->Fs / 400,
                st->channels, st->Fs);
      if (prefill == 3) {
        for (int i = 0; i < prefill_offset; ++i) {
          stage_storage.active[i] *= .75f;
        }
      } else {
  ...
  if (auto* history = celt_enc->quality_history; history != nullptr && (st->use_dtx || st->silk_mode.useInBandFEC))
    quality_history_invalidate_optional(*history, quality_tracking_status::packet_rewritten);
  if (st->mode != opus_mode_silk_only) {
    ...
    if (auto* history = celt_enc->quality_history; history != nullptr) {
      const bool mode_changed = history->packet_prev_mode > 0 && history->packet_prev_mode != st->mode;
      const bool known_reset = history->packet_prev_mode == 0 || (mode_changed && !history->packet_prev_redundancy);
      const bool recoverable_redundancy = redundancy && celt_to_silk && mode_changed && !history->packet_prev_redundancy;
      if (known_reset && (redundancy == 0 || recoverable_redundancy))
        quality_receiver_reset(*history);
      else if (redundancy)
        quality_history_invalidate(*history, quality_tracking_status::unsupported_redundancy);
    }
    if (!skip_celt_for_dtx && ec_tell(&enc) <= 8 * nb_compr_bytes) {
      const bool retain_main_shadow = st->mode != opus_mode_hybrid || (redundancy && celt_to_silk);
      ret = celt_encode_with_ec(celt_enc, celt_pcm.data(), frame_size, nullptr, nb_compr_bytes, &enc, st->mode == opus_mode_hybrid && allow_stereo_policy(st, frame_size) && st->stereo_recovery_frames > 0 && st->stereo_recovery_frames < 255, retain_main_shadow);
      if (ret < 0) {
        return -3;
      }
  ...
  if (redundancy && !celt_to_silk) {
    ...
    auto* quality_history = celt_enc->quality_history;
    if (quality_history != nullptr) {
      quality_history_invalidate(*quality_history, quality_tracking_status::unsupported_redundancy);
    }
    ...
    if (quality_history != nullptr && !quality_recover_redundancy(*quality_history, data + nb_compr_bytes, redundancy_bytes, N2,
                                                                  celt_enc->end, celt_enc->stream_channels))
      quality_history_invalidate(*quality_history, quality_tracking_status::unsupported_redundancy);
    redundant_rng = celt_enc->rng;
  }
  data--;
  ...
  if (ec_tell(&enc) > (max_data_bytes - 1) * 8) {
    if (celt_enc->quality_history != nullptr) {
      quality_history_invalidate(*celt_enc->quality_history, quality_tracking_status::packet_rewritten);
    }
    if (max_data_bytes < 2) {
      return -2;
    }
  ...
  if (auto* history = celt_enc->quality_history; history != nullptr && st->mode == opus_mode_celt_only &&
                                                 history->ready && history->frames == quality_frames_before && st->use_vbr && ret > 1 && !redundancy) {
    if (!quality_recover_final_celt(*history, data + 1, ret - 1, frame_size, celt_enc->end, st->stream_channels))
      quality_history_invalidate(*history, quality_tracking_status::packet_rewritten);
  }
...
[[nodiscard]] static auto ensure_voice_denoise_state(OpusEncoder* st) noexcept -> bool {
  if (st->voice_denoise == nullptr) {
    st->voice_denoise = static_cast<VoiceDenoiseState*>(std::malloc(sizeof(VoiceDenoiseState)));
    if (st->voice_denoise != nullptr) {
      reset_voice_denoise_state(*st->voice_denoise);
    }
  }
  return st->voice_denoise != nullptr;
}

static void release_voice_denoise_state(OpusEncoder* st) noexcept {
  if (st != nullptr) {
    std::free(st->voice_denoise);
    st->voice_denoise = nullptr;
  }
}

struct voice_denoise_coefficients {
  opus_val16 low, middle;
};

[[nodiscard]] static constexpr auto voice_denoise_filter_coefficients(opus_int32 sample_rate) noexcept -> voice_denoise_coefficients {
  switch (sample_rate) {
  case 8000:
    return {.low = .20991872f, .middle = .91899741f};
  case 12000:
    return {.low = .14536400f, .middle = .84000230f};
  case 16000:
    return {.low = .11113483f, .middle = .74702048f};
  case 24000:
    return {.low = .07553475f, .middle = .60000285f};
  default:
    return {.low = .03850884f, .middle = .36754672f};
  }
}
...
static void apply_voice_denoise(OpusEncoder* st, opus_res* pcm, int frame_size, const frame_activity_metrics& metrics) noexcept {
  ...
  const bool noise_shape = metrics.energy > 2e-4f && metrics.energy < .03f && metrics.mono_diff_ratio > .40f &&
                           metrics.mono_zero_cross_rate > .22f;
  ...
  state->evidence_samples = noise_shape && stable_high_band
                                ? std::min(8 * reference_frame_size, state->evidence_samples + frame_size)
                                : std::max(0, state->evidence_samples - frame_size);
  if (state->evidence_samples >= confirmation_samples) {
    state->hold_samples = (st->bitrate_bps <= 16000 ? 1 : 5) * reference_frame_size;
  } else if (state->hold_samples > 0) {
    state->hold_samples = std::max(0, state->hold_samples - frame_size);
  }

  for (int band = 0; band < 3; ++band) {
    if (!state->initialized) {
      state->noise_energy[band] = energy[band];
    } else {
      const auto rate = energy[band] < state->noise_energy[band] ? .25f : noise_shape && stable_high_band ? .008f
                                                                                                          : .0005f;
  ...
  constexpr std::array<opus_val16, 3> strength{.15f, .42f, .68f};
  const bool protect_voice = st->lightweight_voice_score_Q7 > 72 || metrics.energy < .001f;
...
template <std::size_t Capacity>
static opus_int32 encode_pcm16_input(OpusEncoder* st, const opus_int16* pcm, int frame_size, unsigned char* data, opus_int32 max_data_bytes) {
...
static void reset_ref_encoder_state(OpusEncoder* st, CeltEncoderInternal* celt_enc) {
  ...
  celt_encoder_reset_state(celt_enc);
  if (celt_enc->quality_history != nullptr) {
    quality_history_reset(*celt_enc->quality_history, celt_enc->channels, celt_enc->upsample);
  }
  if (encoder_uses_silk(st->application)) {
    reset_encoder_silk_state(st);
  }
...
static opus_int32 write_packet_frames(packet_frame_set* packet_frames, unsigned char* data, opus_int32 maxlen, int pad) {
  ...
  const int code = compact && count == 1 ? 0 : compact && count == 2 ? (vbr ? 2 : 1)
                                                                     : 3;
...
template <bool Encode>
static void process_energy_finalise(int start, int end, celt_glog* oldEBands, celt_glog* error, const int* fine_quant, const int* fine_priority, int bits_left, ec_ctx* coder, int C);
...
static void denormalise_bands(std::array<const celt_norm*, Channels> normalized, std::array<celt_sig*, Channels> frequency, std::array<const celt_glog*, Channels> band_log_energy, int start, int end, int M, int downsample, int silence) {
  const opus_int16* eBands = celt_mode()->eBands;
  const int N = M * celt_short_mdct_size;
...
static void anti_collapse(celt_norm* X_, unsigned char* collapse_masks, int LM, int C, int size, int start, int end, const celt_glog* logE, const celt_glog* prev1logE, const celt_glog* prev2logE, const int* pulses, opus_uint32 seed) {
  ...
  for (int i = start; i < end; i++) {
    ...
    for (int c = 0; c < C; ++c) {
      ...
      celt_norm r = 2.f * static_cast<float>(std::exp(0.6931471805599453094 * (-Ediff)));
      if (LM == 3) {
        r *= 1.41421356f;
      }
      r = std::min(thresh, r) * sqrt_1;
...
static void compute_theta(band_ctx* ctx, split_ctx* sctx, celt_norm* X, celt_norm* Y, int N, int* b, int B, int B0, int LM, int stereo, int* fill) {
...
static unsigned quant_partition(band_ctx* ctx, celt_norm* X, int N, int b, int B, celt_norm* lowband, int LM, opus_val32 gain, int fill, const unsigned char* cache) {
  int q, curr_bits;
  int B0 = B;
  unsigned cm = 0;
  celt_norm* Y = nullptr;
...
static unsigned quant_band(band_ctx* ctx, celt_norm* X, int N, int b, int B, celt_norm* lowband, int LM, celt_norm* lowband_out, opus_val32 gain, celt_norm* lowband_scratch, int fill) {
...
static unsigned quant_band_stereo(band_ctx* ctx, celt_norm* X, celt_norm* Y, int N, int b, int B, celt_norm* lowband, int LM, celt_norm* lowband_out, celt_norm* lowband_scratch, int fill) {
  unsigned cm = 0;
  int mbits, sbits;
  const int encode = ctx->encode;
  auto* ec = ctx->ec;
...
static void celt_commit_band_state(celt_glog* old_band, celt_glog* old_log, celt_glog* old_log2, int channels, int start, int end, bool is_transient, bool mirror_mono) {
...
static inline int compute_vbr(opus_int32 base_target, int LM, opus_int32 bitrate, int lastCodedBands, int C, int intensity, int constrained_vbr, opus_val16 stereo_saving, int tot_boost, celt_glog maxDepth, celt_glog temporal_vbr, bool content_vbr) {
  ...
  if (C == 2) {
    ...
    const auto saving = (stereo_saving - .1f) * static_cast<opus_val32>(coded_stereo_dof << 3);
...
[[nodiscard]] static inline auto celt_preemphasise_input(CeltEncoderInternal* st, const opus_res* pcm, celt_sig* in, celt_sig* prefilter_mem, int N) -> celt_input_metrics {
  ...
  if (st->upsample == 1) {
    ...
    if (frame_max <= 65536.f) {
      ...
      metrics.release = tail_max < 0.125f * sample_max;
      metrics.strong_release = tail_max < 0.03125f * sample_max;
  ...
  metrics.release = st->overlap_max < 0.125f * sample_max;
  metrics.strong_release = st->overlap_max < 0.03125f * sample_max;
  metrics.silence = sample_max <= static_cast<opus_val16>(1) / (1 << st->lsb_depth);
...
[[nodiscard]] static inline auto celt_update_temporal_vbr(CeltEncoderInternal* st, const celt_glog* bandLogE, int LM, int shortBlocks) noexcept -> celt_glog {
...
static int celt_balance_lowrate_stereo_trim(int alloc_trim, const celt_glog* bandLogE, int end, int LM, int effectiveBytes, opus_int32 total_boost, std::span<const int> offsets) {
...
static int celt_encode_candidate(CeltEncoderInternal* st, const opus_res* pcm, int frame_size, unsigned char* compressed, int nbCompressedBytes, ec_enc* enc, bool protect_transients, bool challenger, int fixed_budget = 0, const CeltEncoderInternal* budget_state = nullptr, quality_frame_work* quality_work = nullptr, const ec_enc* quality_checkpoint = nullptr, std::span<const unsigned char> baseline_packet = {}, const std::array<double, 7>* baseline_score = nullptr, celt_analysis_checkpoint* analysis = nullptr, bool resume_analysis = false) {
  ...
  bool input_strong_release = false;
  ...
  std::array<celt_glog, celt_max_channels * celt_default_nb_ebands> error_storage;
  ...
  if (resume_analysis) {
    ...
    input_strong_release = analysis->strong_release != 0;
    ...
    freq = analysis->frequency.data();
    bandE = analysis->bands.data();
    bandLogE = analysis->bands.data() + celt_max_channels * nbEBands;
    bandLogE2 = error_storage.data();
    ...
    input_strong_release = input_metrics.strong_release;
    if (tell == 1)
    ...
    if (analysis != nullptr && !challenger && !release_intervention) {
      ...
      analysis->strong_release = input_strong_release ? 1 : 0;
  ...
  const bool compare_allocation = challenger && quality_history != nullptr && quality_work != nullptr && quality_work->active &&
                                  quality_main_packet && quality_history->profile > 0 && st->vbr && st->constrained_vbr &&
                                  proposal_content && start == 0 && end == nbEBands && LM > 0 && st->complexity >= 1 &&
                                  2 * (8 * std::min(effectiveBytes, nbAvailableBytes) - (40 * C + 20)) >= C * eBands[end] * (1 << LM);
  auto alternate_offsets = offsets;
  opus_int32 alternate_boost = tot_boost;
  std::array<float, celt_max_channels * celt_default_nb_ebands> input_log;
  if (compare_allocation) {
    std::copy_n(bandLogE, C * nbEBands, input_log.data());
  ...
  quant_coarse_energy(start, end, bandLogE, oldBandE, total_bits, error, enc, C, LM, nbAvailableBytes,
                      st->prediction_disabled || (refresh && challenger) ||
                          (input_strong_release && st->bitrate <= 48000),
                      &st->delayedIntra);
  ...
  if (tell + (6 << 3) <= total_bits - total_boost) {
    ...
    if (input_strong_release && st->bitrate <= 48000)
      alloc_trim = 0;
  ...
  if (challenger && min_allowed > fixed_budget)
    return OPUS_BUFFER_TOO_SMALL;
  if (challenger && vbr_rate > 0) {
    nbCompressedBytes = fixed_budget;
    nbAvailableBytes = fixed_budget;
    st->vbr_reservoir = budget_state->vbr_reservoir;
    st->vbr_drift = budget_state->vbr_drift;
    st->vbr_offset = budget_state->vbr_offset;
    st->vbr_count = budget_state->vbr_count;
    ec_enc_shrink(enc, nbCompressedBytes);
  } else if (vbr_rate > 0) {
...
static int celt_encode_with_history(CeltEncoderInternal* st, const opus_res* pcm, int frame_size, unsigned char* compressed, int capacity, ec_enc* enc, bool protect_transients) {
  ...
  if (checkpoint_ready)
    quality_checkpoint = *enc;
  const bool stereo_audio = st->channels == 2 && st->stream_channels == 2 && st->audio_application;
  const bool mono_voip = st->channels == 1 && st->stream_channels == 1 && st->voip_application;
  const bool eligible = quality_history != nullptr && quality_history->profile > 0 && quality_history->borrowed_input != nullptr &&
  ...
  if (!eligible) {
    ...
    const int result = celt_encode_candidate(st, pcm, frame_size, compressed, capacity, enc, protect_transients, false, 0, nullptr, work,
                                             checkpoint_ready ? &quality_checkpoint : nullptr);
    if (result > 0 && quality_history != nullptr && work != nullptr && work->active && work->decoded_ready)
      quality_advance_filters(*quality_history, *work, work->model.state.stream_channels, work->channels, work->samples);
    if (result > 0 && quality_history != nullptr && work != nullptr)
      quality_commit_history(*quality_history, *work);
    if (result > 0 && quality_history != nullptr && work != nullptr && !work->decoded_ready)
      quality_history_invalidate(*quality_history, quality_tracking_status::unsupported_configuration);
    return result;
  }
  ...
  const bool both_survive = alternative == ordinary && alternative_work.active && alternative_work.decoded_ready &&
                            ...
                            alternative_work.score[1] <= ordinary_work.score[1];
  if (!both_survive) {
    restore(baseline);
    quality_advance_filters(*quality_history, ordinary_work, ordinary_work.model.state.stream_channels,
  ...
  if (accept) {
    ...
    (void)feedback;
    for (int i = 0; i < st->channels * celt_default_nb_ebands; ++i)
      if (energy[i] != baseline_energy[i])
...
static int celt_encode_with_ec(CeltEncoderInternal* st, const opus_res* pcm, int frame_size, unsigned char* compressed, int capacity, ec_enc* enc, bool protect_transients, bool retain_main_shadow) {
  if (!retain_main_shadow && st->quality_history != nullptr) {
    quality_history_invalidate(*st->quality_history, quality_tracking_status::hybrid_only);
...
static void celt_synthesis(celt_norm* X, celt_sig* const* out_syn, celt_glog* oldBandE, int start, int effEnd, int C, int CC, int isTransient, int LM, int downsample, int silence) {
...
static void celt_apply_postfilter(CeltDecoderInternal* st, celt_sig* const* out_syn, int channels, int N, int LM, int target_period, opus_val16 target_gain, int target_tapset) {
...
static void quality_snapshot_celt(const OpusDecoder* decoder, quality_celt_snapshot& snapshot) {
  const auto* state = decoder_celt_state(const_cast<OpusDecoder*>(decoder));
  std::memcpy(&snapshot.state, state, sizeof(CeltDecoderInternal));
  std::memcpy(snapshot.storage.data(), celt_decoder_storage(const_cast<CeltDecoderInternal*>(state)),
              celt_decoder_storage_count(state->channels) * sizeof(celt_sig));
}

static void quality_restore_celt(OpusDecoder* decoder, const quality_celt_snapshot& snapshot) {
  auto* state = decoder_celt_state(decoder);
  std::memcpy(state, &snapshot.state, sizeof(CeltDecoderInternal));
  std::memcpy(celt_decoder_storage(state), snapshot.storage.data(), celt_decoder_storage_count(snapshot.state.channels) * sizeof(celt_sig));
  decoder->stream_channels = snapshot.state.stream_channels;
}

static void quality_history_reset(quality_history_state& history, int channels, int upsample) {
  const int profile = history.profile;
  auto* decoder = history.decoder;
  history = {};
  history.profile = profile;
  history.decoder = decoder;
  if (!is_supported_channel_count(channels) || upsample != 1) {
    history.status = quality_tracking_status::unsupported_configuration;
    return;
  }
  if (decoder == nullptr || opus_decoder_ctl(decoder, 4028) != OPUS_OK) {
    history.status = quality_tracking_status::unsupported_configuration;
    return;
  }
  history.ready = true;
  history.status = quality_tracking_status::waiting;
}

static void quality_receiver_reset(quality_history_state& history) {
  history.reference_bands = {};
  history.decoded_bands = {};
  history.incoming_reference_bands = {};
  history.incoming_bands = {};
  history.packet_selection_ready = false;
  history.previous_packet_celt = false;
  celt_decoder_reset_state(decoder_celt_state(history.decoder));
  history.ready = !history.optional_epoch_invalid;

  if (history.ready)
    history.status = quality_tracking_status::waiting;
}

static void quality_history_invalidate(quality_history_state& history, quality_tracking_status status) {
  history.ready = false;
  history.reference_bands = {};
  history.decoded_bands = {};
  history.incoming_reference_bands = {};
...
static void quality_history_invalidate_optional(quality_history_state& history, quality_tracking_status status) {
  quality_history_invalidate(history, status);
  history.optional_epoch_invalid = true;
}

static auto quality_prepare_history(quality_history_state& history, quality_frame_work& work, int start, int end, int C, int CC) -> bool {
  const bool supported = history.ready && !history.optional_epoch_invalid && (start == 0 || start == 17) && end <= celt_default_nb_ebands && C >= 1 && C <= CC &&
                         history.decoder != nullptr && CC == decoder_celt_state(history.decoder)->channels;
  if (!supported) {
    if (history.ready)
      quality_history_invalidate(history, quality_tracking_status::unsupported_configuration);
    return false;
  }
  work.active = true;
  work.analysis_ready = history.packet_selection_ready;
  return true;
}

[[nodiscard]] static float quality_reference_sample(const quality_history_state& history, int sample, int channel) {
  if (sample < quality_input_delay_48k_audio)
    return history.input_delay[channel][sample];
  const int offset = history.borrowed_channels * (sample - quality_input_delay_48k_audio) + channel;
  return history.borrowed_float_input != nullptr ? history.borrowed_float_input[offset] : history.borrowed_input[offset] * (1.f / 32768);
}
...
static bool quality_probe_real_decoder(quality_history_state& history, quality_frame_work& work, const ec_enc& checkpoint,
  ...
  auto* decoder = &work.model.state;
  decoder->start = start;
  decoder->end = end;
  decoder->stream_channels = C;
  ec_dec entropy{};
  if (!quality_resume_decoder(checkpoint, {payload, static_cast<std::size_t>(bytes)}, entropy))
    return false;
  const int decoded = celt_decode_with_ec(decoder, payload, bytes, work.output.data(), frame_size, &entropy);
  if (decoded != frame_size || entropy.error != 0)
    return false;
  work.samples = decoded;
  work.channels = decoder->channels;
  work.decoded_ready = true;
  const bool input_ready = (history.borrowed_input != nullptr || history.borrowed_float_input != nullptr) &&
                           history.borrowed_frame_size == decoded && history.borrowed_channels == decoder->channels &&
                           C >= 1 && C <= decoder->channels;
  if (!input_ready)
    return true;
  double square_error = 0, absolute_error = 0;
  for (int begin = 0; begin < decoded; begin += 120) {
    for (int sample = begin; sample < std::min(begin + 120, decoded); ++sample) {
  ...
  work.score = {};
  work.score[0] = square_error;
  work.score[1] = absolute_error;
...
static bool quality_recover_redundancy(quality_history_state& history, unsigned char* payload, int bytes, int frame_size, int end, int stream_channels) {
  if (bytes <= 1 || frame_size <= 0 || frame_size > celt_max_frame_samples)
    return false;
  quality_receiver_reset(history);
  history.decoder->stream_channels = stream_channels;
  auto* decoder = decoder_celt_state(history.decoder);
  decoder->start = 0;
  decoder->end = end;
  decoder->stream_channels = stream_channels;
  std::array<float, celt_max_frame_samples * celt_max_channels> output{};
  if (celt_decode_with_ec(decoder, payload, bytes, output.data(), frame_size, nullptr) != frame_size)
    return false;
  history.status = quality_tracking_status::unsupported_transition;
  return true;
}

static bool quality_recover_final_celt(quality_history_state& history, unsigned char* payload, int bytes, int frame_size, int end, int stream_channels) {
  if (bytes <= 1 || frame_size <= 0 || frame_size > celt_max_frame_samples)
    return false;
  auto* owner = history.decoder;
  owner->stream_channels = stream_channels;
  auto* decoder = decoder_celt_state(owner);
  decoder->start = 0;
  decoder->end = end;
  decoder->stream_channels = stream_channels;
  ...
  work.channels = decoder->channels;
  ec_dec entropy{};
  ec_dec_init(&entropy, payload, static_cast<opus_uint32>(bytes));
  const int decoded = celt_decode_with_ec(decoder, payload, bytes, work.output.data(), frame_size, &entropy);
  if (decoded != frame_size || entropy.error != 0)
    return false;
  work.samples = decoded;
  work.decoded_ready = true;
  work.score = quality_score_decoded(history, work, stream_channels, decoder->channels, decoded);
  quality_snapshot_celt(owner, work.model);
  quality_commit_history(history, work);
  return true;
}

static void quality_advance_filters(quality_history_state& context, quality_frame_work& work, int C, int CC, int N) {
  ...
  for (int b = 0; b < 8; ++b)
    coefficients[b] = 1 - std::exp(-6.283185307179586 * cutoffs[b] / 48000);
  for (int begin = 0; begin < N; begin += 120) {
    for (int j = begin; j < std::min(begin + 120, N); ++j) {
      for (int c = 0; c < CC; ++c) {
        const double target = quality_reference_sample(context, j, c), sample = work.output[j * CC + c];
        for (int b = 0; b < 8; ++b) {
          auto& r = reference_state[c][b];
          auto& d = decoded_state[c][b];
...
static void quality_commit_history(quality_history_state& history, const quality_frame_work& work) {
  if (!work.active || !work.decoded_ready)
    return;
  quality_restore_celt(history.decoder, work.model);
  history.reference_bands = work.reference_bands;
  history.decoded_bands = work.decoded_bands;
  ++history.frames;
  history.status = work.model.state.start == 0 ? quality_tracking_status::valid : quality_tracking_status::hybrid_only;
}
...
static int celt_decode_with_ec(CeltDecoderInternal* st, const unsigned char* data, int len, opus_res* pcm, int frame_size, ec_dec* dec, opus_int16* pcm16) {
  ec_dec _dec;
  const int CC = st->channels;
  const int C = st->stream_channels;
  constexpr int nbEBands = celt_default_nb_ebands;
  ...
  const int silence = tell >= total_bits ? 1 : tell == 1 ? ec_dec_bit_logp(dec, 15)
                                                         : 0;
  if (silence) {
    tell = len * 8;
  ...
  if (!intra_ener && st->loss_duration != 0) {
    for (int c = 0; c < energy_channels; ++c) {
      const celt_glog safety = LM == 0 ? 1.5f : LM == 1 ? .5f
                                                        : 0;
  ...
  st->rng = dec->rng;
  if (pcm16 != nullptr && st->downsample == 1) {
    if (CC == 1) {
...
void pfa_split_radix_butterflies(kiss_fft_cpx& a0, kiss_fft_cpx& a1, kiss_fft_cpx& a2, kiss_fft_cpx& a3, float t1, float t2, float t5, float t6) noexcept {
  const float r0 = a0.r;
  const float i0 = a0.i;
  const float r1 = a1.r;
  const float i1 = a1.i;
...
void pfa_split_radix_transform(kiss_fft_cpx& a0, kiss_fft_cpx& a1, kiss_fft_cpx& a2, kiss_fft_cpx& a3, float real, float imaginary) noexcept {
...
void pfa_fft3(const kiss_fft_cpx& in0, const kiss_fft_cpx& in1, const kiss_fft_cpx& in2, kiss_fft_cpx& out0, kiss_fft_cpx& out1, kiss_fft_cpx& out2) noexcept {
...
consteval void fill_fft_bitrev(std::array<opus_int16, Size>& values, int output, std::size_t index, std::size_t stride, const std::array<int, FactorCount>& factors, std::size_t level, int remaining) noexcept {
...
static int interp_bits2pulses(int start, int end, int skip_start, const int* bits1, const int* bits2, const int* thresh, const int* cap, opus_int32 total, opus_int32* _balance, int skip_rsv, int* intensity, int intensity_rsv, int* dual_stereo, int dual_stereo_rsv, int* bits, int* ebits, int* fine_priority, int C, int LM, ec_ctx* ec, int encode, int prev, int signalBandwidth) {
...
static unsigned normalise_residual_and_extract_collapse_mask(const opus_int16* iy, celt_norm* X, int N, int B, opus_val32 Ryy, opus_val32 gain) {
...
static opus_int32 stereo_itheta(const celt_norm* X, const celt_norm* Y, int stereo, int N) {
  ...
  const auto angle = side * side + mid * mid < 1e-18f ? 0.0f : side < mid ? celt_atan_norm(side / mid)
                                                                          : 1.0f - celt_atan_norm(mid / side);
...
static void silk_decode_core(silk_decoder_state& state, silk_decoder_control& control, opus_int16 xq[], const opus_int16 pulses[((5 * 4) * 16)]) {
  int lag = 0;
...
int silk_Decode(void* decState, silk_DecControlStruct* decControl, int lostFlag, int newPacketFlag, ec_dec* psRangeDec, opus_res* samplesOut, opus_int32* nSamplesOut) {
  ...
  *nSamplesOut = static_cast<opus_int32>((nSamplesOutDec * decControl->API_sampleRate) / (mid.fs_kHz * 1000));
  ...
  for (int n = 0; n < std::min(decControl->nChannelsAPI, decControl->nChannelsInternal); ++n) {
    silk_resample_and_store_channel(*channel_state[n], samplesOut2_tmp, &samplesOut1_tmp[n][1], nSamplesOutDec, samplesOut,
...
[[nodiscard]] static auto silk_nsq_build_sample(opus_int32 q_Q10, opus_int32 seed, opus_int32 LTP_pred_Q14, opus_int32 LPC_pred_Q14, opus_int32 x_Q10, opus_int32 n_AR_Q14, opus_int32 n_LF_Q14) noexcept -> silk_nsq_sample_state;
...
static void silk_quant_LTP_gains(opus_int16 B_Q14[4 * 5], opus_uint8 cbk_index[4], opus_uint8* periodicity_index, opus_int32* sum_log_gain_Q7, int* pred_gain_dB_Q7, const opus_int32 XX_Q17[4 * 5 * 5], const opus_int32 xX_Q17[4 * 5], const int subfr_len, const int nb_subfr) {
  int j, k, cbk_size;
...
void silk_VQ_WMat_EC_c(opus_uint8* ind, opus_int32* res_nrg_Q15, opus_int32* rate_dist_Q8, int* gain_Q7, const opus_int32* XX_Q17, const opus_int32* xX_Q17, const opus_int8* cb_Q7, const opus_uint8* cb_gain_Q7, const opus_uint8* cl_Q5, const int subfr_len, const opus_int32 max_gain_Q7, const int L) {
...
static void silk_NLSF_encode(std::span<opus_int8, 17> NLSFIndices, std::span<opus_int16, 16> pNLSF_Q15, const silk_NLSF_CB_struct* psNLSF_CB, std::span<const opus_int16, 16> pW_Q2, const int NLSF_mu_Q20, const int nSurvivors, const int signalType) {
...
[[nodiscard]] constexpr auto silk_nlsf_rd_q25(opus_int32 base_Q25, int in_Q10, opus_int16 out_Q10, opus_int16 weight_Q5, opus_int32 mu_Q20, int rate_Q5) noexcept -> opus_int32 {
...
opus_int32 silk_NLSF_del_dec_quant(std::span<opus_int8, 16> indices, std::span<const opus_int16, 16> x_Q10, std::span<const opus_int16, 16> w_Q5, std::span<const opus_uint8, 16> pred_coef_Q8, std::span<const opus_int16, 16> ec_ix, const opus_uint8 ec_rates_Q5[], const int quant_step_size_Q16, const opus_int16 inv_quant_step_size_Q6, const opus_int32 mu_Q20, const opus_int16 order) {
...
static void silk_biquad_alt_stride1(const opus_int16* in, const opus_int32 B_Q28[3], const opus_int32 A_Q28[2], opus_int32 S[2], opus_int16* out, const opus_int32 len) {
...
[[nodiscard]] constexpr auto resampler_fractional_fir18_q6(std::span<const opus_int32, 18> buffer, std::span<const opus_int16, 9> leading, std::span<const opus_int16, 9> trailing) noexcept -> opus_int32 {
...
[[nodiscard]] constexpr auto resampler_symmetric_fir_q6(std::span<const opus_int32, 2 * HalfOrder> buffer, std::span<const opus_int16, HalfOrder> coefficients) noexcept -> opus_int32 {
  auto acc = opus_int32{0};
...
template <std::size_t HalfOrder>
static auto resample_symmetric_fir(opus_int16* out, const opus_int32* buffer, const opus_int16* coefficients, opus_int32 max_index_Q16, opus_int32 index_increment_Q16) noexcept -> opus_int16* {
...
static opus_int32 silk_stereo_find_predictor(opus_int32& ratio_Q14, const opus_int16 x[], const opus_int16 y[], std::span<opus_int32, 2> mid_res_amp_Q0, int length, int smooth_coef_Q16) {
...
void silk_stereo_LR_to_MS(stereo_enc_state* state, opus_int16 x1[], opus_int16 x2[], silk_stereo_pred_indices& ix, opus_uint8* mid_only_flag, opus_int32 mid_side_rates_bps[], opus_int32 total_rate_bps, int prev_speech_act_Q8, int toMono, int preserve_stereo, int fs_kHz, int frame_length) {
  ...
  *mid_only_flag = 0;
  ...
  const bool was_mid_only = state->width_prev_Q14 == 0;
  if (toMono) {
  ...
  } else if (state->smth_width_Q14 > fixed_q<14>(0.95)) {
    silk_stereo_quant_pred(std::span<opus_int32, 2>{pred_Q13}, ix);
    width_Q14 = 1 << 14;
  } else {
    quantize_smoothed_pred();
...
[[nodiscard]] auto dispatch_encoder_control(OpusEncoder* st, int request, va_list& ap) noexcept -> int {
  ...
  case OPUSCPP_SET_VOICE_DENOISE_REQUEST: {
    const auto value = va_arg(ap, opus_int32);
    if (value < 0 || value > 1) {
      return OPUS_BAD_ARG;
    }
  ...
  case OPUSCPP_GET_VOICE_DENOISE_REQUEST:
    return ctl_write_value(ap, static_cast<opus_int32>(st->voice_denoise != nullptr));
...
[[nodiscard]] auto dispatch_decoder_control(OpusDecoder* st, int request, va_list& ap) noexcept -> int {
  ...
  case OPUSCPP_GET_DECODE_POSTFILTER_REQUEST:
    return ctl_write_value(ap, opus_int32{0});
  case OPUS_RESET_STATE: {
    auto* silk_dec = decoder_silk_state(st);
    auto* celt_dec = decoder_celt_state(st);
    celt_decoder_reset_state(celt_dec);
...
void opus_encoder_destroy(OpusEncoder* st) noexcept {
  ...
  release_voice_denoise_state(st);
  if (st != nullptr)
    release_quality_history(encoder_celt_state(st));
  std::free(st);
}

...
inline constexpr int OPUS_GET_LOOKAHEAD_REQUEST = 4027;
...
#define OPUS_GET_LOOKAHEAD(x) OPUS_GET_LOOKAHEAD_REQUEST, (x)
#define OPUS_GET_IN_DTX(x) OPUS_GET_IN_DTX_REQUEST, (x)
...
int opus_decode(OpusDecoder* st, std::span<const unsigned char> packet, std::span<opus_int16> pcm, int decode_fec = 0) noexcept;
